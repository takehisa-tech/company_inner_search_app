...
RETRIEVER_FETCH_K = 50  # MMR検索時に最初に取得する候補数（多めに設定）
RETRIEVER_LAMBDA_MULT = 0.5  # MMRの多様性パラメータ（0=多様性重視、1=関連性重視）
...
SYSTEM_PROMPT_INQUIRY = """
    ...
    【条件】
    1. 以下の文脈に含まれる情報を最大限活用して、ユーザーの質問に答えてください。
    2. 文脈に関連する情報が少しでもあれば、それを使って回答を作成してください。
    ...
    7. リスト形式や表形式での回答が適切な場合は、そのような形式を使用してください。
